select pro.name                                                                                                    as pro_name,
       ...
       case
         when trans.movementdate is null then null
       ...
       p_cate.name                                                                                                 as cate_name,
       p_cate.m_product_category_id                                                                                as cate_id,
       pro.value                                                                                                   as pro_value,
       ...
       sum(case
             when trans.movementtype in ('I+', 'P+', 'V+', 'C+', 'W+', 'M+') then trans.movementqty
             else 0 end)                                                                                           as amout_in,

       sum(case
             when trans.movementtype in ('I-', 'P-', 'V-', 'C-', 'W-', 'M-') then -1 * trans.movementqty
             else 0 end)                                                                                           as amout_out,
...
from (select AD_Org_ID,
             ...
             m_productionline_id
      from m_transaction
      where movementdate <= current_date + 10
        and ad_client_id = 1000000
        and m_product_id in (select DISTINCT m_product_id
                             from m_transaction
                             where movementdate >= current_date - 10
                               and movementdate <= current_date + 10
                               and ad_client_id = 1000000)
     ) as trans
       ...
       left outer join (select m_inventoryline.m_inventoryline_id,
                               m_inventory.movementdate,
                               m_inventory.documentno,
                               c_doctype.name as doc_name
       ...
       left outer join (select m_movementline.m_movementline_id,
       ...
       left outer join (select m_inoutline.m_inoutline_id,
       ...
       left outer join (select m_productionline.m_productionline_id,
                               m_production.movementdate,
                               m_production.documentno,
                               'production'::varchar as doc_name
                        from m_productionline
                               inner join m_production
                                          on m_production.m_production_id = m_productionline.m_production_id
) as doc_pro on trans.m_productionline_id = doc_pro.m_productionline_id

Group By trans.movementdate, p_cate.name, p_cate.m_product_category_id, pro.name, pro.m_product_id, pro.value,
         case
           when trans.movementdate is null then null
           when trans.m_inventoryline_id is not null then trans.m_inventoryline_id
           when trans.m_movementline_id is not null then trans.m_movementline_id
...
ORDER BY pro.name, move_date NULLS first, doc_no
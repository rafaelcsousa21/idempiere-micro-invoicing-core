...
public class MMovement extends X_M_Movement implements DocAction, IPODoc {
  ...
  public MMovementLine[] getLines(boolean requery) {
    ...
    List<MMovementLine> list =
        new Query(getCtx(), I_M_MovementLine.Table_Name, whereClause, get_TrxName())
  ...
  public MMovementConfirm[] getConfirmations(boolean requery) {
    ...
    List<MMovementConfirm> list =
        new Query(getCtx(), I_M_MovementConfirm.Table_Name, "M_Movement_ID=?", get_TrxName())
            .setParameters(getId())
            .list();
  ...
  public void setProcessed(boolean processed) {
    ...
    int noLine = executeUpdateEx(sql, new Object[] {processed, getId()}, get_TrxName());
    m_lines = null;
      ...
      if (line.getMAttributeSetInstance_ID() == 0) {
        if (product != null && product.isASIMandatory(true)) {
          if (!product
            ...
            MMovementLineMA mas[] =
                MMovementLineMA.get(getCtx(), line.getM_MovementLine_ID(), get_TrxName());
        ...
        if (product != null && product.isStocked()) {
          ...
          if (!isReversal()) {
            BigDecimal qtyOnLineMA =
                MMovementLineMA.getManualQty(line.getM_MovementLine_ID(), get_TrxName());
  ...
  private MMovement reverse(boolean accrual) {
    ...
    for (int i = 0; i < oLines.length; i++) {
      ...
      if (rLine.getMAttributeSetInstance_ID() == 0) {
        MMovementLineMA mas[] =
            MMovementLineMA.get(getCtx(), oLine.getM_MovementLine_ID(), get_TrxName());
        for (int j = 0; j < mas.length; j++) {
          MMovementLineMA ma =
              new MMovementLineMA(
                  rLine,

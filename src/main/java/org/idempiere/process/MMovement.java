...
public class MMovement extends X_M_Movement implements DocAction, IPODoc {
            ...
            if (line.getMAttributeSetInstance_ID() == 0) {
                if (product != null && product.isASIMandatory(true)) {
                    if (!product
                        ...
                        MMovementLineMA mas[] =
                                MMovementLineMA.get(getCtx(), line.getM_MovementLine_ID());
                ...
                if (product != null && product.isStocked()) {
                    ...
                    if (!isReversal()) {
                        BigDecimal qtyOnLineMA =
                                MMovementLineMA.getManualQty(line.getM_MovementLine_ID());
                    ...
                    if (line.getMAttributeSetInstance_ID() == 0) {
                        ...
                        for (int j = 0; j < mas.length; j++) {
                            ...
                            //
                            MLocator locator = new MLocator(getCtx(), line.getM_Locator_ID());
                            // Update Storage
    ...
    private void checkMaterialPolicy(MMovementLine line, BigDecimal qtyToDeliver) {
        ...
        int no = MMovementLineMA.deleteMovementLineMA(line.getM_MovementLine_ID());
    ...
    private MMovement reverse(boolean accrual) {
        ...
        for (int i = 0; i < oLines.length; i++) {
            ...
            if (rLine.getMAttributeSetInstance_ID() == 0) {
                MMovementLineMA mas[] =
                        MMovementLineMA.get(getCtx(), oLine.getM_MovementLine_ID());
                for (int j = 0; j < mas.length; j++) {
                    MMovementLineMA ma =
                            new MMovementLineMA(
                                    rLine,

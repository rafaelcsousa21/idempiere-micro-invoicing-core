/**
 * **************************************************************************** Product: Adempiere
 * ERP & CRM Smart Business Solution * Copyright (C) 1999-2006 Adempiere, Inc. All Rights Reserved.
 * * This program is free software; you can redistribute it and/or modify it * under the terms
 * version 2 of the GNU General Public License as published * by the Free Software Foundation. This
...
import static software.hsharp.core.util.DBKt.isOracle;
        ...
        try {
            ...
            sql =
                    "UPDATE AD_WF_NODE n"
                            ...
                            + (isOracle() ? "dbms_lob.substr(n.Help, 4000, 1 )" : "n.Help")
                            + ",' ')))";
            no = executeUpdate(sql);
            if (log.isLoggable(Level.INFO)) log.info("  rows updated: " + no);
            ...
            sql =
                    "UPDATE AD_WF_NODE_TRL trl"
                            ...
                            + (isOracle() ? "dbms_lob.substr(trl.Help, 4000, 1 )" : "trl.Help")
                            + ",' ') <> NVL(t.Help,' ')))";
            no = executeUpdate(sql);
            if (log.isLoggable(Level.INFO)) log.info("  rows updated: " + no);

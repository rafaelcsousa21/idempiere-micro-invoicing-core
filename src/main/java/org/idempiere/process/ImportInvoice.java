...
public class ImportInvoice extends SvrProcess {
  ...
  protected String doIt() throws java.lang.Exception {
    ...
    sql =
        new StringBuilder("SELECT * FROM I_Invoice ")
    ...
    try {
      ...
      while (rs.next()) {
          ...
          if (invoice != null) {
            if (!invoice.processIt(m_docAction)) {
              ...
              throw new IllegalStateException(
                  "Invoice Process Failed: " + invoice + " - " + invoice.getProcessMsg());
            }
            invoice.saveEx();
          }
          //	Group Change
        ...
        if (price != null && Env.ZERO.compareTo(price) != 0) line.setPrice(price);
        ...
        if (taxAmt != null && Env.ZERO.compareTo(taxAmt) != 0) line.setTaxAmt(taxAmt);

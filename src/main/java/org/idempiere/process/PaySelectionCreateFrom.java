...
public class PaySelectionCreateFrom extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        if (p_DueDate == null) p_DueDate = psel.getPayDate();

        //	psel.getPayDate();

        StringBuilder sql =
                new StringBuilder("SELECT C_Invoice_ID,") // 1
                        //	Open
        ...
        sql.append(" AND IsPaid='N' AND DocStatus IN ('CO','CL')")
                ...
                .append(
                        " AND i.C_Invoice_ID NOT IN (SELECT i.C_Invoice_ID FROM C_PaySelectionLine psl WHERE psl.C_PaySelection_ID=?)"); //	##p9
        //	Disputed
        if (!p_IncludeInDispute) sql.append(" AND i.IsInDispute='N'");
        //	PaymentRule (optional)
        if (p_PaymentRule != null) sql.append(" AND PaymentRule=?"); // 	##
        //	OnlyDiscount
        if (p_OnlyDiscount) {
            if (p_OnlyDue) sql.append(" AND (");
            else sql.append(" AND ");
            sql.append("invoiceDiscount(i.C_Invoice_ID,?,i.C_InvoicePaySchedule_ID) > 0"); // 	##
        }
        //	OnlyDue
        if (p_OnlyDue) {
            if (p_OnlyDiscount) sql.append(" OR ");
            else sql.append(" AND ");
            // sql.append("paymentTermDueDays(C_PaymentTerm_ID, DateInvoiced, ?) >= 0");	//	##
            sql.append("i.DueDate<=?"); // 	##
            if (p_OnlyDiscount) sql.append(")");
        }
        //	Business Partner
        ...
        try {
            ...
            if (p_OnlyDue) pstmt.setTimestamp(index++, p_DueDate);
            ...
            while (rs.next()) {
                ...
                pselLine.setInvoice(
                        ...
                        isSOTrx,
                        PayAmt,
                        PayAmt.subtract(DiscountAmt).subtract(WriteOffAmt),
                        DiscountAmt,
                        WriteOffAmt);
                if (!pselLine.save()) {

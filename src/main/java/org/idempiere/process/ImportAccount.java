...
import org.compiere.accounting.X_I_ElementValue;
...
public class ImportAccount extends SvrProcess {
  ...
  protected String doIt() throws java.lang.Exception {
    ...
    String[] yColumns =
        new String[] {"PostActual", "PostBudget", "PostStatistical", "PostEncumbrance"};
    for (int i = 0; i < yColumns.length; i++) {
      sql =
          new StringBuilder("UPDATE I_ElementValue SET ")
              .append(yColumns[i])
              .append("='Y' WHERE ")
              .append(yColumns[i])
              .append(" IS NULL OR ")
    ...
    //
    sql =
        new StringBuilder("SELECT COALESCE(i.ParentElementValue_ID, 0), i.I_ElementValue_ID,")
    ...
    try {
      ...
      // PreparedStatement updateStmt = prepareStatement(updateSQL, get_TrxName());
      PreparedStatement updateStmt =
          prepareStatement(
              updateSQL, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE, get_TrxName());
      // end
      //
      while (rs.next()) {
    ...
    //	Update Description
    sql =
        new StringBuilder("SELECT * FROM C_ValidCombination vc ")
  ...
  private void updateDefaults(String clientCheck) {
    ...
    //	****	Update Defaults
    StringBuilder sql =
        new StringBuilder("SELECT C_AcctSchema_ID FROM C_AcctSchema_Element ")
            .append("WHERE C_Element_ID=?")
            .append(clientCheck);
    PreparedStatement pstmt = null;
    ResultSet rs = null;
    try {
    ...
    sql =
        new StringBuilder("UPDATE C_AcctSchema_Element e ")
  ...
  /**
   * Update Default Accounts. _Default.xxxx = C_ValidCombination_ID => Account_ID=C_ElementValue_ID
   *
  ...
  /**
   * Update Default Account. This is the sql to delete unused accounts - with the import still in
   * the table(!): DELETE C_ElementValue e WHERE NOT EXISTS (SELECT * FROM Fact_Acct f WHERE
   * f.Account_ID=e.C_ElementValue_ID) AND NOT EXISTS (SELECT * FROM C_ValidCombination vc WHERE
   * vc.Account_ID=e.C_ElementValue_ID) AND NOT EXISTS (SELECT * FROM I_ElementValue i WHERE
   * i.C_ElementValue_ID=e.C_ElementValue_ID);
   *
   ...
   */
  private int updateDefaultAccount(
      String TableName, String ColumnName, int C_AcctSchema_ID, int C_ElementValue_ID) {
    StringBuilder msglog =
    ...
    StringBuilder sql =
        new StringBuilder("SELECT x.")
            .append(ColumnName)
            .append(",Account_ID FROM ")
            .append(TableName)
            .append(" x INNER JOIN C_ValidCombination vc ON (x.")
            .append(ColumnName)
      ...
      if (rs.next()) {
              ...
              if (C_ValidCombination_ID != newC_ValidCombination_ID) {
                ...
                msglog =
                    new StringBuilder("New #")
                        ...
                        .append(newC_ValidCombination_ID);
                if (log.isLoggable(Level.FINE)) log.fine(msglog.toString());
                if (no == 1) retValue = UPDATE_YES;
              }
            } else log.log(Level.SEVERE, "Account not saved - " + acct);
          } else //	Replace Combination
          {
            //	Only Acct Combination directly
            sql =
                new StringBuilder("UPDATE C_ValidCombination SET Account_ID=")
                    .append(C_ElementValue_ID)
            ...
            if (no == 1) {
              ...
              if (log.isLoggable(Level.FINE)) log.fine(msglog.toString());
              sql =
                  new StringBuilder("UPDATE Fact_Acct SET Account_ID=")
                      .append(C_ElementValue_ID)
                      .append(" WHERE Account_ID=")
                      .append(Account_ID);
              no = executeUpdate(sql.toString(), get_TrxName());
        ...
        if (new MColumn(getCtx(), columnID, get_TrxName()).isActive())
          ...
          log.log(
              ...
              "The account "
                  + ColumnName
                  + " is deprecated, you should consider removing it from your import file");
      }
    } catch (SQLException e) {
      log.log(Level.SEVERE, sql.toString(), e);
    } finally {

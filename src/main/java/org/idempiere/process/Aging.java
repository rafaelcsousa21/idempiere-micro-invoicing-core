...
public class Aging extends SvrProcess {
    ...
    //FR 1933937
    String dateacct = TO_DATE(p_StatementDate);
    ...
    sql.append(",oi.C_Activity_ID,oi.C_Campaign_ID,oi.C_Project_ID,oi.orgId ");	//	14..17
    if (!p_DateAcct)//FR 1933937
    {
    ...
    try
    {
    	...
    	while (rs.next())
    	{
    		...
    		if (aging == null 		//	Key
    			...
    			|| C_InvoicePaySchedule_ID != aging.getInvoicePayScheduleId())
    		{
    			if (aging != null)
    			{
    				aging.saveEx();

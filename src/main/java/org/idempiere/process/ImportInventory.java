...
import org.compiere.accounting.MProductCategoryAcct;
 ...
 *     <p>Contributor: Carlos Ruiz - globalqss - IDEMPIERE-281 Extend Import Inventory to support
 *     also internal use Deepak Pansheriya - logilite - IDEMPIERE-2314 Making import inventory
 *     process extendible
 */
public class ImportInventory extends SvrProcess implements ImportProcess {
  ...
  protected void prepare() {
    ...
    for (int i = 0; i < para.length; i++) {
      ...
      else if (name.equals("IsUpdateCosting")) p_UpdateCosting = "Y".equals(para[i].getParameter());
      ...
      else log.log(Level.WARNING, "Unknown Parameter: " + name);
    }
  } //	prepare
  ...
  protected String doIt() throws java.lang.Exception {
    StringBuilder msglog =
        new StringBuilder("M_Locator_ID=")
            .append(p_M_Locator_ID)
            .append(",MovementDate=")
            .append(p_MovementDate);
    ...
    sql =
        new StringBuilder("SELECT * FROM I_Inventory ")
      ...
      while (rs.next()) {
        ...
        if (inventory == null
          ...
          if (inventory != null) {
            ...
            ModelValidationEngine.get()
                .fireImportValidate(this, lastImp, inventory, ImportValidator.TIMING_AFTER_IMPORT);

            if (m_docAction != null && m_docAction.length() > 0) {
              if (!inventory.processIt(m_docAction)) {
                log.warning(
                    "Inventory Process Failed: " + inventory + " - " + inventory.getProcessMsg());
          ...
          ModelValidationEngine.get()
              .fireImportValidate(this, imp, inventory, ImportValidator.TIMING_BEFORE_IMPORT);
          //
          if (!inventory.save()) {
        ...
        ModelValidationEngine.get()
            .fireImportValidate(this, imp, line, ImportValidator.TIMING_AFTER_IMPORT);
        lastImp = imp;
      }
      if (inventory != null) {
        if (m_docAction != null && m_docAction.length() > 0) {
  ...
  protected int generateASI(MProduct product, X_I_Inventory imp) {
    ...
    if ((imp.getLot() != null && imp.getLot().length() > 0)
        || (imp.getSerNo() != null && imp.getSerNo().length() > 0)) {

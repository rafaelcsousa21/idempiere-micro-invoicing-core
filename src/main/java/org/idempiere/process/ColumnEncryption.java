 ...
 * Adempiere ERP Bazaar * http://www.adempiere.org * * Copyright (C) 1999 - 2006 Compiere Inc. *
 ...
 * 51 Franklin Street, Fifth Floor, Boston, * MA 02110-1301, USA. * * Contributors: * - Bahman
 * Movaqar (bmovaqar AT users.sf.net) *
 * ********************************************************************
 */
...
public class ColumnEncryption extends SvrProcess {
  ...
  /** All the resizing and encrypting database are managed by this transaction. */
  private Trx m_trx;

  /** All the resizing and encrypting database work goes through this connection. */
  ...
  protected void prepare() {
    ...
    for (int i = 0; i < para.length; i++) {
      ...
      else if (name.equals("IsEncrypted")) p_IsEncrypted = "Y".equals(para[i].getParameter());
      else if (name.equals("ChangeSetting")) p_ChangeSetting = "Y".equals(para[i].getParameter());
      else if (name.equals("MaxLength")) p_MaxLength = para[i].getParameterAsInt();
      else if (name.equals("TestValue")) p_TestValue = (String) para[i].getParameter();
  ...
  protected String doIt() throws Exception {
    ...
    if (column.isKey()
      ...
      StringBuilder msgreturn =
          new StringBuilder().append(columnName).append(": cannot be encrypted");
      throw new Exception(msgreturn.toString());
    }

    // Start
    StringBuilder msglog =
        new StringBuilder("Encryption Class = ").append(SecureEngine.getClassName());
    ...
    if (p_IsEncrypted && p_TestValue != null && p_TestValue.length() > 0) {
      ...
      if (p_TestValue.equals(clearString)) {
        msglog =
            new StringBuilder("Decrypted=").append(clearString).append(" (same as test value)");
        addLog(0, null, null, msglog.toString());
      } else {
        msglog =
      ...
      msglog =
          new StringBuilder("Test Length=")
              .append(p_TestValue.length())
              .append(" -> ")
              .append(encLength);
      addLog(0, null, null, msglog.toString());
      if (encLength <= column.getFieldLength()) {
        msglog =
            new StringBuilder("Encrypted Length (")
                .append(encLength)
                .append(") fits into field (")
                .append(column.getFieldLength())
                .append(")");
        addLog(0, null, null, msglog.toString());
      } else {
        msglog =
    ...
    if (p_IsEncrypted && p_MaxLength != 0) {
      ...
      while (testClear.length() < p_MaxLength) testClear.append(testClear);
      testClear.delete(p_MaxLength, testClear.length());
      msglog =
          new StringBuilder()
              .append("Test=")
              .append(testClear.toString())
              .append(" (")
              .append(p_MaxLength)
              .append(")");
      ...
      int encLength = encString.length();
      msglog =
          new StringBuilder("Test Max Length=")
              .append(testClear.length())
              .append(" -> ")
              .append(encLength);
      addLog(0, null, null, msglog.toString());
      if (encLength <= column.getFieldLength()) {
        msglog =
            new StringBuilder("Encrypted Max Length (")
                .append(encLength)
                .append(") fits into field (")
                .append(column.getFieldLength())
                .append(")");
    ...
    if (p_ChangeSetting && column.isEncrypted() != p_IsEncrypted) {
      ...
      if (p_IsEncrypted) {
        ...
        if (newLength > oldLength) {
          if (changeFieldLength(columnID, columnName, newLength, tableName) == -1) {
            log.warning(
                "EncryptError [ChangeFieldLength]: "
                    + "ColumnID="
                    + columnID
                    + ", NewLength="
                    + newLength);
      ...
      if (error || !p_ChangeSetting) {
        msglog =
            new StringBuilder("Encryption NOT changed - Encryption=").append(column.isEncrypted());
        addLog(0, null, null, msglog.toString());
      } else {
        ...
        if (column.save()) {
          ...
          msglog =
              new StringBuilder("Encryption CHANGED - Encryption=").append(column.isEncrypted());
          addLog(0, null, null, msglog.toString());
        } else addLog(0, null, null, "Save Error");
      }
    } else {
      addLog(
          0,
          ...
          null,
          "Can't perform "
              + (p_IsEncrypted ? "encryption. " : "decryption. ")
              + "Column is "
              + (p_IsEncrypted ? "already Encrypted." : " not Encrypted."));
    }
   ...
   *
   * @param columnName The ID of the column to be encrypted.
   * @param tableName The name of the table which owns the column.
   * @return The number of rows effected or -1 in case of errors.
   */
  private int encryptColumnContents(String columnName, String tableName) throws Exception {
    ...
    try {
      String selectSqlStr = getDatabase().convertStatement(selectSql.toString());
      selectStmt =
          m_conn.prepareStatement(
              selectSqlStr, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
   ...
   *
   * @param currentColSize Current column size
   ...
   */
  private int changeFieldLength(int columnID, String columnName, int length, String tableName)
      throws Exception {
    int rowsEffected = -1;
    ...
    try {
      selectSql = getDatabase().convertStatement(selectSql);
      selectStmt =
          m_conn.prepareStatement(
      ...
      if (rs.next()) {
        ...
        alterSql = new StringBuilder(getDatabase().convertStatement(alterSql.toString()));
        if (executeUpdate(alterSql.toString(), false, m_trx.getTrxName()) == -1) {
          log.severe(
              "EncryptError [ChangeFieldLength]: ColumnID=" + columnID + ", NewLength=" + length);
          throw new Exception();
        }

        // Change the column size in AD.
        updateSql = new StringBuilder(getDatabase().convertStatement(updateSql.toString()));
        if (executeUpdate(updateSql.toString(), false, m_trx.getTrxName()) == -1) {
          log.severe(
              "EncryptError [ChangeFieldLength]: ColumnID=" + columnID + ", NewLength=" + length);
          throw new Exception();
        }

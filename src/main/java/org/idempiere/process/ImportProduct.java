    ...
    try {
      StringBuilder dbpst =
          new StringBuilder("SELECT C_TaxCategory_ID FROM C_TaxCategory WHERE IsDefault='Y'")
              .append(clientCheck);
      pstmt = prepareStatement(dbpst.toString(), get_TrxName());
      ...
      while (rs.next()) {
        ...
        if (p_M_PriceList_Version_ID != 0) {
          ...
          if (PriceStd.signum() != 0 || PriceLimit.signum() != 0 || PriceList.signum() != 0) {
            MProductPrice pp =
                MProductPrice.get(getCtx(), p_M_PriceList_Version_ID, M_Product_ID, get_TrxName());
            if (pp == null)
              pp =
                  new MProductPrice(
                      getCtx(), p_M_PriceList_Version_ID, M_Product_ID, get_TrxName());

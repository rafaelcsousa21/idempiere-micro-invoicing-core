...
public class ImportProduct extends SvrProcess implements ImportProcess {
  ...
  protected String doIt() throws java.lang.Exception {
    ...
    sql =
        new StringBuilder("UPDATE I_Product ")
    ...
    ModelValidationEngine.get()
        .fireImportValidate(this, null, null, ImportValidator.TIMING_BEFORE_VALIDATE);

    //	Set Optional BPartner
    sql =
        new StringBuilder("UPDATE I_Product i ")
            .append("SET C_BPartner_ID=(SELECT C_BPartner_ID FROM C_BPartner p")
            .append(" WHERE i.BPartner_Value=p.Value AND i.clientId=p.clientId) ")
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            ...
            .append(" AND i.VendorProductNo=p.VendorProductNo AND i.clientId=p.clientId) ")
            .append("WHERE M_Product_ID IS NULL")
            .append(" AND I_IsImported='N'")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    sql =
        new StringBuilder("UPDATE I_Product ")
            ...
            .append(" WHERE IsDefault='Y' AND clientId=")
            .append(m_AD_Client_ID)
            .append(") ")
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            ...
            .append(" WHERE i.ProductCategory_Value=c.Value AND i.clientId=c.clientId) ")
    ...
    for (int i = 0; i < strFieldsToCopy.length; i++) {
      sql =
          new StringBuilder("UPDATE I_Product i ")
              .append("SET ")
              .append(strFieldsToCopy[i])
              .append(" = (SELECT ")
              .append(strFieldsToCopy[i])
              .append(" FROM M_Product p")
              .append(" WHERE i.M_Product_ID=p.M_Product_ID AND i.clientId=p.clientId) ")
    ...
    for (int i = 0; i < strFieldsPO.length; i++) {
      sql =
          new StringBuilder("UPDATE I_PRODUCT i ")
              ...
              .append(
                  " WHERE i.M_Product_ID=p.M_Product_ID AND i.C_BPartner_ID=p.C_BPartner_ID AND i.clientId=p.clientId) ")
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            .append("SET X12DE355 = ")
            .append(
                "(SELECT MAX(X12DE355) FROM C_UOM u WHERE u.IsDefault='Y' AND u.clientId IN (0,i.clientId)) ")
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            .append(
                "SET C_UOM_ID = (SELECT C_UOM_ID FROM C_UOM u WHERE u.X12DE355=i.X12DE355 AND u.clientId IN (0,i.clientId)) ")
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            ...
            .append(" WHERE ci.clientId=i.clientId) ")
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            ...
            .append("WHERE I_IsImported<>'Y'")
            .append(
                " AND Value IN (SELECT Value FROM I_Product ii WHERE i.clientId=ii.clientId GROUP BY Value HAVING COUNT(*) > 1)")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            ...
            .append("WHERE I_IsImported<>'Y'")
            .append(
                " AND UPC IN (SELECT UPC FROM I_Product ii WHERE i.clientId=ii.clientId GROUP BY UPC HAVING COUNT(*) > 1)")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    //		+ " AND VendorProductNo IS NULL AND (C_BPartner_ID IS NOT NULL OR BPartner_Value IS NOT
    // NULL)").append(clientCheck);
    //	no = executeUpdate(sql.toString(), null);
    ...
    sql =
        new StringBuilder("UPDATE I_Product i ")
            ...
            .append(
                " (SELECT C_BPartner_ID, VendorProductNo FROM I_Product ii WHERE i.clientId=ii.clientId GROUP BY C_BPartner_ID, VendorProductNo HAVING COUNT(*) > 1)")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    try {
      ...
      PreparedStatement pstmt_insertProduct = conn.prepareStatement
      	...
      	+ "SELECT ?,"
      	+ "clientId,orgId,'Y',SysDate,CreatedBy,SysDate,UpdatedBy,"
      	+ "Value,Name,Description,DocumentNote,Help,"
      ...
      */
      ...
      /*
      			String sqlt = "UPDATE M_PRODUCT "
      				...
      				+ "WHERE M_Product_ID=?";
      			PreparedStatement pstmt_updateProduct = prepareStatement
      				(sqlt, null);
      			...
      			PreparedStatement pstmt_updateProductPO = prepareStatement
      				(sqlt, null);
      */
      //	Insert Product from Import
      pstmt_insertProductPO =
          prepareStatement(
      ...
      while (rs.next()) {
        ...
        if (newProduct) //	Insert new Product
        {
          ...
          ModelValidationEngine.get()
              .fireImportValidate(this, imp, product, ImportValidator.TIMING_AFTER_IMPORT);
          if (product.save()) {
        ...
        {
          StringBuilder sqlt =
              new StringBuilder("UPDATE M_PRODUCT ")
                  ...
                  .append(" FROM I_Product WHERE I_Product_ID=")
                  .append(I_Product_ID)
                  .append(") ")
          ...
          if (!newProduct) {
            StringBuilder sqlt =
                new StringBuilder("UPDATE M_Product_PO ")
                    ...
                    .append("(SELECT CAST('Y' AS CHAR),C_UOM_ID,C_Currency_ID,UPC,") // jz fix EDB
                    // unknown datatype
                    // error
                    .append("PriceList,PricePO,RoyaltyAmt,PriceEffective,")
                    .append("VendorProductNo,VendorCategory,Manufacturer,")
  ...
  public String getWhereClause() {
    StringBuilder msgreturn = new StringBuilder(" AND clientId=").append(m_AD_Client_ID);

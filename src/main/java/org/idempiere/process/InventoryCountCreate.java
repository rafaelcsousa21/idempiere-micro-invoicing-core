...
public class InventoryCountCreate extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        if (log.isLoggable(Level.INFO))
            log.info(
                    "M_Inventory_ID="
                            ...
                            + ", DeleteOld="
                            + p_DeleteOld);
        m_inventory = new MInventory(getCtx(), p_M_Inventory_ID);
        ...
        StringBuilder sql =
                new StringBuilder("SELECT s.M_Product_ID, s.M_Locator_ID, s.M_AttributeSetInstance_ID,");
        ...
        //
        if (p_M_Locator_ID != 0) sql.append(" AND s.M_Locator_ID=?");
        //
        ...
        if (p_ProductValue != null
            ...
            p_ProductValue = null;
        if (p_ProductValue != null) sql.append(" AND UPPER(p.Value) LIKE ?");
        //
        ...
        try {
            ...
            if (!p_DeleteOld) pstmt.setInt(index++, p_M_Inventory_ID);
    ...
    private int createInventoryLine(
        ...
        if (m_line != null
            ...
            //
            MInventoryLineMA ma =
                    new MInventoryLineMA(m_line, M_AttributeSetInstance_ID, QtyOnHand, dateMPolicy, true);
            if (!ma.save()) log.warning("Could not save " + ma);
            return 0;
        }
        //	new line

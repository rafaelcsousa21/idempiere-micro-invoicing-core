...
public class ImportConversionRate extends SvrProcess {
  ...
  protected void prepare() {
    ...
    for (int i = 0; i < para.length; i++) {
      ...
      else if (name.equals("ValidFrom")) p_ValidFrom = (Timestamp) para[i].getParameter();
  ...
  protected String doIt() throws Exception {
    StringBuilder msglog =
        new StringBuilder("doIt - clientId=")
            ...
            .append(",ValidFrom=")
            .append(p_ValidFrom)
            .append(",CreateReciprocalRate=")
            .append(p_CreateReciprocalRate);
    ...
    if (p_C_ConversionType_ID != 0)
      sql.append(" C_ConversionType_ID = COALESCE (C_ConversionType_ID,")
          ...
          .append("),");
    if (p_ValidFrom != null)
      sql.append(" ValidFrom = COALESCE (ValidFrom,").append(TO_DATE(p_ValidFrom)).append("),");
    else sql.append(" ValidFrom = COALESCE (ValidFrom,SysDate),");
    sql.append(" CreateReciprocalRate = COALESCE (CreateReciprocalRate,'")
        .append(p_CreateReciprocalRate ? "Y" : "N")
        .append("'),")
    ...
    //	Org
    sql =
        new StringBuilder("UPDATE I_Conversion_Rate o ")
    ...
    sql =
        new StringBuilder("UPDATE I_Conversion_Rate i ")
            ...
            .append(
                " WHERE c.Value=i.ConversionTypeValue AND c.clientId IN (0,i.clientId) AND c.IsActive='Y') ")
    ...
    sql =
        new StringBuilder("UPDATE I_Conversion_Rate i ")
            ...
            .append(
                "	WHERE c.ISO_Code=i.ISO_Code AND c.clientId IN (0,i.clientId) AND c.IsActive='Y') ")
    ...
    sql =
        new StringBuilder("UPDATE I_Conversion_Rate i ")
            ...
            .append(
                "	WHERE c.ISO_Code=i.ISO_Code_To AND c.clientId IN (0,i.clientId) AND c.IsActive='Y') ")
    ...
    int noInsert = 0;
    sql =
        new StringBuilder("SELECT * FROM I_Conversion_Rate ")

...
public class ImportBankStatement extends SvrProcess {
  ...
  protected String doIt() throws java.lang.Exception {
    ...
    if (p_deleteOldImported) {
      sql =
          new StringBuilder("DELETE I_BankStatement ")
    ...
    sql =
        new StringBuilder("UPDATE I_BankStatement o ")
    ...
    //	Set Bank Account
    sql =
        new StringBuilder("UPDATE I_BankStatement i ")
    ...
    sql =
        new StringBuilder("UPDATE I_BankStatement ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) if (log.isLoggable(Level.INFO)) log.info("Interest Amount=" + no);
    //
    sql =
        new StringBuilder("UPDATE I_BankStatement ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) if (log.isLoggable(Level.INFO)) log.info("Transaction Amount=" + no);
    //
    sql =
        new StringBuilder("UPDATE I_BankStatement ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) if (log.isLoggable(Level.INFO)) log.info("Invalid Amount=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_BankStatement ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) if (log.isLoggable(Level.INFO)) log.info("Valuta Date=" + no);
    ...
    //	Detect Duplicates
    sql =
        new StringBuilder("SELECT i.I_BankStatement_ID, l.C_BankStatementLine_ID, i.EftTrxID ")
    ...
    StringBuilder updateSql =
        new StringBuilder("UPDATE I_Bankstatement ")
            ...
            .append("WHERE I_BankStatement_ID=?")
            .append(clientCheck);
    PreparedStatement pupdt = prepareStatement(updateSql.toString(), null);

    PreparedStatement pstmtDuplicates = null;
    ResultSet rs = null;
    no = 0;
    try {
      pstmtDuplicates = prepareStatement(sql.toString(), null);
      rs = pstmtDuplicates.executeQuery();
      while (rs.next()) {
        StringBuilder info =
            new StringBuilder("Line_ID=")
                .append(rs.getInt(2)) // 	l.C_BankStatementLine_ID
                .append(",EDTTrxID=")
                .append(rs.getString(3)); // 	i.EftTrxID
      ...
      close(rs, pstmtDuplicates);
      rs = null;
      pstmtDuplicates = null;
      close(pupdt);
      pupdt = null;
    }
    if (no != 0) if (log.isLoggable(Level.INFO)) log.info("Duplicates=" + no);

    // Import Bank Statement
    sql =
        new StringBuilder("SELECT * FROM I_BankStatement")
    ...
    try {
      ...
      while (rs.next()) {
        ...
        else if ((statement.getEftStatementReference() != null)
            && (imp.getEftStatementReference() != null)) {
          if (!statement.getEftStatementReference().equals(imp.getEftStatementReference())) {
            statement = null;
            msglog =
                new StringBuilder("New Statement, Statement Reference=")
                    .append(imp.getEftStatementReference());
            if (log.isLoggable(Level.INFO)) log.info(msglog.toString());
          }
        }
        //	Create a new Bank Statement for every statement date
          ...
          if (!statement.getStatementDate().equals(imp.getStatementDate())) {
            statement = null;
            msglog =
                new StringBuilder("New Statement, Statement Date=").append(imp.getStatementDate());
            if (log.isLoggable(Level.INFO)) log.info(msglog.toString());
          }
        }

 ...
 * 95054, USA * or via info@compiere.org or http://www.compiere.org/license.html * Contributor:
...
public class ImportGLJournal extends SvrProcess {
  ...
  protected void prepare() {
    ...
    for (int i = 0; i < para.length; i++) {
      ...
      else if (name.equals("DateAcct")) m_DateAcct = (Timestamp) para[i].getParameter();
      else if (name.equals("IsValidateOnly")) m_IsValidateOnly = "Y".equals(para[i].getParameter());
  ...
  protected String doIt() throws java.lang.Exception {
    StringBuilder msglog =
        new StringBuilder("IsValidateOnly=")
            .append(m_IsValidateOnly)
            .append(", IsImportOnlyNoErrors=")
            .append(m_IsImportOnlyNoErrors);
    ...
    if (m_DeleteOldImported) {
      sql =
          new StringBuilder("DELETE I_GLJournal ")
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            .append(
                "SET clientId=(SELECT c.clientId FROM AD_Client c WHERE c.Value=i.ClientValue) ")
            .append("WHERE (clientId IS NULL OR clientId=0) AND ClientValue IS NOT NULL")
    ...
    if (m_C_AcctSchema_ID != 0)
      sql.append(" C_AcctSchema_ID = COALESCE (C_AcctSchema_ID,")
          .append(m_C_AcctSchema_ID)
          .append("),");
    if (m_DateAcct != null)
      sql.append(" DateAcct = COALESCE (DateAcct,").append(TO_DATE(m_DateAcct)).append("),");
    ...
    //	Error Doc Org
    sql =
        new StringBuilder("UPDATE I_GLJournal o ")
            .append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Doc Org, '")
            .append("WHERE (AD_OrgDoc_ID IS NULL OR AD_OrgDoc_ID=0")
            .append(
                " OR EXISTS (SELECT * FROM AD_Org oo WHERE o.AD_OrgDoc_ID=oo.orgId AND (oo.IsSummary='Y' OR oo.IsActive='N')))")
            .append(" AND I_IsImported<>'Y'")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid Doc Org=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid AcctSchema=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid GLCategory=" + no);

    //	Set Currency
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            .append("SET C_Currency_ID=(SELECT c.C_Currency_ID FROM C_Currency c")
            .append(" WHERE c.ISO_Code=i.ISO_Code AND c.clientId IN (0,i.clientId)) ")
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(
                " WHERE c.Value=i.ConversionTypeValue AND c.clientId IN (0,i.clientId)) ")
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid CurrencyTypeValue=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("No CourrencyType=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(" AND C_Currency_ID IS NOT NULL AND I_IsImported<>'Y'")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("No Rate=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid Period=" + no);
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Period Closed=" + no);

    //	Posting Type
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            .append("SET PostingType='A' ")
            .append("WHERE PostingType IS NULL AND I_IsImported<>'Y'")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid PostingTypee=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            .append("SET orgId=AD_OrgDoc_ID ")
            .append(
                "WHERE (orgId IS NULL OR orgId=0) AND OrgValue IS NULL AND AD_OrgDoc_ID IS NOT NULL AND AD_OrgDoc_ID<>0")
            .append(
                " AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'")
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid Account=" + no);
    ...
    // tbayen: IDEMPIERE-539 Import GL Window does not allow Key Values for Activity, Campaign,
    // Sales Region
    //	Set Campaign
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid Campaign=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid SalesRegion=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Invalid OrgTrx=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Zero Source Balance=" + no);
    ...
    sql =
        new StringBuilder("UPDATE I_GLJournal i ")
            ...
            .append(clientCheck);
    no = executeUpdate(sql.toString(), null);
    if (no != 0) log.warning("Zero Acct Balance=" + no);
    // AZ Goodwill
    ...
    */
    ...
    try {
      ...
      if (rs.next()) {
        ...
        if (source != null && source.signum() == 0 && acct != null && acct.signum() == 0) {
          ...
          msglog =
              new StringBuilder("Balance Source=").append(source).append(", Acct=").append(acct);
          log.warning(msglog.toString());
        }
        if (source != null) addLog(0, null, source, "@AmtSourceDr@ - @AmtSourceCr@");
        if (acct != null) addLog(0, null, acct, "@AmtAcctDr@ - @AmtAcctCr@");
      }
    ...
    //	Count Errors
    StringBuilder msgdb =
        new StringBuilder("SELECT COUNT(*) FROM I_GLJournal WHERE I_IsImported NOT IN ('Y','N')")
            .append(clientCheck);
    int errors = getSQLValue(null, msgdb.toString());

    if (errors != 0) {
      if (m_IsValidateOnly || m_IsImportOnlyNoErrors) throw new Exception("@Errors@=" + errors);
    ...
    sql =
        new StringBuilder("SELECT * FROM I_GLJournal ")
            .append("WHERE I_IsImported='N'")
            .append(clientCheck)
            .append(
                " ORDER BY COALESCE(BatchDocumentNo, TO_NCHAR(I_GLJournal_ID)||' '), COALESCE(JournalDocumentNo, ")
            .append(
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (imp.isCreateNewBatch() // line states to create a new batch
            || (journal != null
                && journal.getC_AcctSchema_ID()
                    != imp.getC_AcctSchema_ID()) // new line changed schema
            || (imp.getBatchDocumentNo() == null
                && !wasCreateNewBatch) // new line doesn't have batch info
        ...
        if (imp.isCreateNewBatch()) wasCreateNewBatch = true;
        ...
        if (imp.isCreateNewBatch() || (batch == null && imp.getBatchDocumentNo() != null)) {
          ...
          if (imp.getBatchDescription() == null
              || imp.getBatchDescription().toString().length() == 0)
            description = new StringBuilder("*Import-");
          else description = new StringBuilder(imp.getBatchDescription()).append(" *Import-");
        ...
        if (impJournalDocumentNo == null) impJournalDocumentNo = "";
        ...
        } else line.setC_ValidCombination_ID(imp.getC_ValidCombination_ID());
        //
        ...
        //
        if (line.save()) {
          if (batch != null) imp.setGL_JournalBatch_ID(batch.getGL_JournalBatch_ID());

...
public class ModelValidationEngine {
  ...
  /**
   * Called when setSecurityContext is complete
   *
  ...
  public String loginComplete(int AD_Client_ID, int AD_Org_ID, int AD_Role_ID, int AD_User_ID) {
    ...
    // now process the script model validator setSecurityContext
    ...
    if (AD_User_ID == 0 && AD_Role_ID == 0) ; // don't validate for user system on role system
    else if (!Util.isEmpty(missingModelValidationMessage)) {
      MSystem system = MSystem.get(Env.getCtx());
      if (system.isFailOnMissingModelValidator()) return missingModelValidationMessage;
    }
    return null;
  } //	loginComplete
  ...
  public void addModelChange(String tableName, ModelValidator listener) {
    ...
    String propertyName =
        m_globalValidators.contains(listener)
            ? tableName + "*"
            : tableName + listener. getClientId();
  ...
  public String fireModelChange(PO po, int changeType) {
    ...
    List<MTableScriptValidator> scriptValidators =
        MTableScriptValidator.getModelValidatorRules(
            po.getCtx(), po.getTableId(), ModelValidator.tableEventValidators[changeType]);
      ...
      for (MTableScriptValidator scriptValidator : scriptValidators) {
        ...
        if (rule != null
          ...
          try {
            ...
            engine.put(
                MRule.ARGUMENTS_PREFIX + "Event", ModelValidator.tableEventValidators[changeType]);
    ...
    IEvent event =
        EventManager.newEvent(
            ModelValidator.tableEventTopics[changeType],
        ...
        if (validator. getClientId() == po. getClientId()
          ...
          if (error != null && error.length() > 0) {
            if (log.isLoggable(Level.FINE)) {
              log.log(
                  Level.FINE, "po=" + po + " validator=" + validator + " changeType=" + changeType);
            }
            return error;
          }
        }
  ...
  public void removeDocValidate(String tableName, ModelValidator listener) {
    ...
    if (list == null) return;
    list.remove(listener);
    if (list.size() == 0) m_docValidateListeners.remove(propertyName);
  } //	removeDocValidate
  ...
  public void addFactsValidate(String tableName, FactsValidator listener) {
    ...
    //
    String propertyName =
        (listener instanceof ModelValidator
  ...
  public void removeFactsValidate(String tableName, FactsValidator listener) {
    ...
    if (list == null) return;
    list.remove(listener);
    if (list.size() == 0) m_factsValidateListeners.remove(propertyName);
  } //	removeFactsValidate
  ...
  public void fireImportValidate(
      ...
      org.idempiere.orm.PO importModel,
      org.idempiere.orm.PO targetModel,
      int timing) {
    ...
    String topic = null;
    if (timing == ImportValidator.TIMING_AFTER_IMPORT) topic = IEventTopics.IMPORT_AFTER_IMPORT;
    ...
    IEvent event =
        EventManager.newEvent(
            ...
            new EventProperty("importTableName", process.getImportTableName()));
    EventManager.getInstance().sendEvent(event);
  }
  ...
  public void afterLoadPreferences(Properties ctx) {
      ...
      if (AD_Client_ID == validator. getClientId() || m_globalValidators.contains(validator)) {
        ...
        try {
          m =
              validator
                  .getClass()
                  .getMethod("afterLoadPreferences", new Class[] {Properties.class});
        } catch (NoSuchMethodException e) {
          // ignore
        }
        try {
          if (m != null) m.invoke(validator, ctx);
        } catch (Exception e) {
    ...
    IEvent event =
        EventManager.getInstance()
            .createNewEvent(IEventTopics.PREF_AFTER_LOAD, (Map<String, ?>) null);
    EventManager.getInstance().sendEvent(event);
  }

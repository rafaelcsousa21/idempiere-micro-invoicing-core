...
public class MQuery implements Serializable {
  ...
  public static MQuery get(Properties ctx, int AD_PInstance_ID, String TableName) {
    ...
    boolean isFinancialReport =
        ("T_Report".equals(TableName) || "T_ReportStatement".equals(TableName));
    query.m_AD_PInstance_ID = AD_PInstance_ID;

    //	How many rows do we have?
    String SQL = "SELECT COUNT(*) FROM AD_PInstance_Para WHERE AD_PInstance_ID=?";
    int rows = getSQLValue(null, SQL, AD_PInstance_ID);

    if (rows < 1) return query;

    //	Msg.getMsg(Env.getCtx(), "Parameter")
    boolean trl = !Env.isBaseLanguage(ctx, "AD_Process_Para");
    if (!trl)
      SQL =
          "SELECT ip.ParameterName,ip.P_String,ip.P_String_To," //	1..3
      ...
      SQL =
          "SELECT ip.ParameterName,ip.P_String,ip.P_String_To, ip.P_Number,ip.P_Number_To,"
    ...
    try {
      ...
      if (trl) pstmt.setString(2, Env.getADLanguage(ctx));
      ...
      for (int row = 0; rs.next(); row++) {
        ...
        if (!rs.wasNull()) P_Number = new Double(d);
        Double P_Number_To = null;
        d = rs.getDouble(5);
        if (!rs.wasNull()) P_Number_To = new Double(d);
        //
        ...
        if (isTemporaryTable
            && !isFinancialReport
            && table != null
            && table.getColumn(ParameterName) == null) {
          if (s_log.isLoggable(Level.INFO))
            s_log.info(
                "Skip parameter "
                    + ParameterName
                    + " because there is no column in table "
                    + TableName);
          continue;
        }
        ...
        if (P_String != null) {
          if (P_String_To == null) {
            ...
            else query.addRestriction(ParameterName, MQuery.LIKE, P_String, Name, Info);
          } else
        ...
        //	Number
          ...
          if (P_Number_To == null) {
            if (isRange)
              query.addRestriction(ParameterName, MQuery.GREATER_EQUAL, P_Number, Name, Info);
            else query.addRestriction(ParameterName, MQuery.EQUAL, P_Number, Name, Info);
          } else //	P_Number_To != null
          {
            if (P_Number == null)
              query.addRestriction(
                  "TRUNC(" + ParameterName + ")", MQuery.LESS_EQUAL, P_Number_To, Name, Info);
            else
        ...
        //	Date
          ...
          if (P_Date_To == null) {
            if (isRange)
              query.addRestriction(
                  "TRUNC(" + ParameterName + ")", MQuery.GREATER_EQUAL, P_Date, Name, Info);
            else
              query.addRestriction(
                  "TRUNC(" + ParameterName + ")", MQuery.EQUAL, P_Date, Name, Info);
          } else //	P_Date_To != null
          {
            if (P_Date == null)
              query.addRestriction(
                  "TRUNC(" + ParameterName + ")", MQuery.LESS_EQUAL, P_Date_To, Name, Info);
            else
              query.addRangeRestriction(
                  "TRUNC(" + ParameterName + ")", P_Date, P_Date_To, Name, Info, Info_To);
          }
        }
      }
  ...
  public static String getZoomColumnName(String columnName) {
    ...
    if (columnName.equals("SalesRep_ID")) return "AD_User_ID";
    if (columnName.equals("C_DocTypeTarget_ID")) return "C_DocType_ID";
    if (columnName.equals("Bill_BPartner_ID")) return "C_BPartner_ID";
    if (columnName.equals("Bill_Location_ID") || columnName.equals("BillTo_ID"))
      return "C_BPartner_Location_ID";
    if (columnName.equals("Account_ID")) return "C_ElementValue_ID";
    if (columnName.equals("C_LocFrom_ID") || columnName.equals("C_LocTo_ID"))
      return "C_Location_ID";
    if (columnName.equals("C_UOM_To_ID")) return "C_UOM_ID";
    if (columnName.equals("M_AttributeSetInstanceTo_ID")) return "M_AttributeSetInstance_ID";
  ...
  public static String getZoomTableName(String columnName) {
    ...
    if (index != -1) return tableName.substring(0, index);
    return tableName;
  } //	getZoomTableName

  /**
   * *********************************************************************** Create simple Equal
   * Query. Creates columnName=value or columnName='value'
   *
  ...
  public void addRestriction(
      ...
      int depth) {
    Restriction r =
        new Restriction(ColumnName, Operator, Code, InfoName, InfoDisplay, andCondition, depth);
    m_list.add(r);
  } //	addRestriction
   ...
   */
  public void addRestriction(
      String ColumnName, String Operator, Object Code, String InfoName, String InfoDisplay) {
   ...
   * @param depth ( = no open brackets )
   */
  public void addRangeRestriction(
      String ColumnName,
      Object Code,
      Object Code_to,
      String InfoName,
    ...
    Restriction r =
        new Restriction(
            ColumnName, Code, Code_to, InfoName, InfoDisplay, InfoDisplay_to, andCondition, depth);
    m_list.add(r);
  } //	addRestriction
  ...
  public void addRangeRestriction(
      ...
      String InfoDisplay_to) {
    Restriction r =
        new Restriction(ColumnName, Code, Code_to, InfoName, InfoDisplay, InfoDisplay_to, true, 0);
    m_list.add(r);
  } //	addRestriction
  ...
  public String getInfo() {
    StringBuilder sb = new StringBuilder();
    int currentDepth = 0;
    if (m_TableName != null) sb.append(m_TableName).append(": ");
    //
    for (int i = 0; i < m_list.size(); i++) {
      Restriction r = (Restriction) m_list.get(i);
      for (; currentDepth < r.joinDepth; currentDepth++) {
  ...
  public String getDisplayName(Properties ctx) {
    ...
    if (m_TableName != null) keyColumn = m_TableName + "_ID";
    else keyColumn = getColumnName(0);
    String retValue = Msg.translate(ctx, keyColumn);
    if (retValue != null && retValue.length() > 0) return retValue;
    return m_TableName;
  } //	getDisplayName
  ...
  public MQuery deepCopy() {
    ...
    for (int i = 0; i < m_list.size(); i++) newQuery.addRestriction((Restriction) m_list.get(i));
    return newQuery;
  } //	clone

  /**
   * @return AD_PInstance_ID; this value is set if you created this query by using {@link

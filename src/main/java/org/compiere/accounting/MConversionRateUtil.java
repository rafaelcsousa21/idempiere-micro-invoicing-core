...
public final class MConversionRateUtil {
   ...
   * @param AmtName converted amount field name (optional); if null then the converted amount field
   *     will not be updated
  ...
  public static BigDecimal convertBase(
      org.compiere.model.SetGetModel model,
      String DateName,
    ...
    int C_Currency_ID_To =
        MClient.get(model.getCtx(), AD_Client_ID).getAcctSchema().getC_Currency_ID();
      ...
      if (C_Currency_ID == C_Currency_ID_To) {
        ...
        rate =
            MConversionRate.getRate(
                ...
                AD_Client_ID,
                AD_Org_ID);
        if (rate == null) { // NoCurrencyConversion
          throw new NoCurrencyConversionException(
              C_Currency_ID,
              C_Currency_ID_To,
              ConvDate,
              C_ConversionType_ID,
    ...
    // Update model
    if (AmtName != null) model.set_AttrValue(AmtName, amt);
    // Return amt
    if (s_log.isLoggable(Level.FINE))
      s_log.fine("amt=" + sourceAmt + " * " + rate + "=" + amt + ", scale=" + stdPrecision);
    return amt;
  } //	convert

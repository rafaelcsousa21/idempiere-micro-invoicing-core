...
public class MPayment extends X_C_Payment
  ...
  public static MPayment[] getOfBPartner(Properties ctx, int C_BPartner_ID, String trxName) {
    ...
    List<MPayment> list =
        new Query(ctx, I_C_Payment.Table_Name, whereClause, trxName)
  ...
  public MPayment(Properties ctx, int C_Payment_ID, String trxName) {
    ...
    if (C_Payment_ID == 0) {
      ...
      //
      setR_AvsAddr(X_C_Payment.R_AVSZIP_Unavailable);
      setR_AvsZip(X_C_Payment.R_AVSZIP_Unavailable);
      //
      setIsReceipt(true);
      setIsApproved(false);
  ...
  /**
   * ************************************************************************ Set Credit Card. Need
   * to set PatmentProcessor after Amount/Currency Set
   *
  ...
  public boolean setCreditCard(
      ...
      int creditCardExpMM,
      int creditCardExpYY) {
    setTenderType(X_C_Payment.TENDERTYPE_CreditCard);
    setTrxType(TrxType);
    //
  ...
  public boolean setBankCash(int C_BankAccount_ID, boolean isReceipt, String tenderType) {
    ...
    setIsReceipt(isReceipt);
    //
    if (C_BankAccount_ID > 0) setBankAccountDetails(C_BankAccount_ID);
    else {
      setC_BankAccount_ID(C_BankAccount_ID);
    }
    //
  ...
  protected boolean beforeSave(boolean newRecord) {
    if (isComplete()
        && !is_ValueChanged(I_C_Payment.COLUMNNAME_Processed)
        && (is_ValueChanged(I_C_Payment.COLUMNNAME_C_BankAccount_ID)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_C_BPartner_ID)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_C_Charge_ID)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_C_Currency_ID)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_C_DocType_ID)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_DateAcct)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_DateTrx)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_DiscountAmt)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_PayAmt)
            || is_ValueChanged(I_C_Payment.COLUMNNAME_WriteOffAmt))) {
    ...
    if (getC_DocType_ID() == 0) setC_DocType_ID();
    else {
  ...
  public BigDecimal getAllocatedAmt() {
    ...
    if (getC_Charge_ID() != 0) return getPayAmt();
    //
    String sql =
  ...
  public static void setIsAllocated(Properties ctx, int C_BPartner_ID, String trxName) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (pay.testAllocation()) if (pay.save()) counter++;
      }
    } catch (Exception e) {
  ...
  public void setC_BankAccount_ID(int C_BankAccount_ID) {
    if (C_BankAccount_ID == 0) {
      setPaymentProcessor();
      if (getC_BankAccount_ID() == 0) throw new IllegalArgumentException("Can't find Bank Account");
    } else super.setC_BankAccount_ID(C_BankAccount_ID);
  } //	setC_BankAccount_ID
  ...
  public ValueNamePair[] getCreditCards(BigDecimal amt) {
    try {
      if (m_mBankAccountProcessors == null || m_mBankAccountProcessors.length == 0)
        m_mBankAccountProcessors =
            MBankAccountProcessor.find(
                getCtx(), null, null,  getClientId(), getC_Currency_ID(), amt, null);
      //
      HashMap<String, ValueNamePair> map =
          new HashMap<String, ValueNamePair>(); // 	to eliminate duplicates
      for (int i = 0; i < m_mBankAccountProcessors.length; i++) {
        MBankAccountProcessor bankAccountProcessor = m_mBankAccountProcessors[i];
        MPaymentProcessor paymentProcessor =
            new MPaymentProcessor(
                getCtx(), bankAccountProcessor.getC_PaymentProcessor_ID(), null);

        if (bankAccountProcessor.isAcceptAMEX() && paymentProcessor.isAcceptAMEX())
          map.put(
              X_C_Payment.CREDITCARDTYPE_Amex, getCreditCardPair(X_C_Payment.CREDITCARDTYPE_Amex));
        if (bankAccountProcessor.isAcceptDiners() && paymentProcessor.isAcceptDiners())
          map.put(
              X_C_Payment.CREDITCARDTYPE_Diners,
              getCreditCardPair(X_C_Payment.CREDITCARDTYPE_Diners));
        if (bankAccountProcessor.isAcceptDiscover() && paymentProcessor.isAcceptDiscover())
          map.put(
              X_C_Payment.CREDITCARDTYPE_Discover,
              getCreditCardPair(X_C_Payment.CREDITCARDTYPE_Discover));
        if (bankAccountProcessor.isAcceptMC() && paymentProcessor.isAcceptMC())
          map.put(
              X_C_Payment.CREDITCARDTYPE_MasterCard,
              getCreditCardPair(X_C_Payment.CREDITCARDTYPE_MasterCard));
        if (bankAccountProcessor.isAcceptCorporate() && paymentProcessor.isAcceptCorporate())
          map.put(
              X_C_Payment.CREDITCARDTYPE_PurchaseCard,
              getCreditCardPair(X_C_Payment.CREDITCARDTYPE_PurchaseCard));
        if (bankAccountProcessor.isAcceptVisa() && paymentProcessor.isAcceptVisa())
          map.put(
              X_C_Payment.CREDITCARDTYPE_Visa, getCreditCardPair(X_C_Payment.CREDITCARDTYPE_Visa));
      ...
      if (log.isLoggable(Level.FINE))
        log.fine(
            "getCreditCards - #"
                + retValue.length
                + " - Processors="
                + m_mBankAccountProcessors.length);
      return retValue;
    } catch (Exception ex) {
      ex.printStackTrace();
      return null;
  ...
  public boolean setCreditCardExp(String mmyy) {
    if (MPaymentValidate.validateCreditCardExp(mmyy).length() != 0) return false;
    //
  ...
  private void setDocumentNo() {
    ...
    if (getR_PnRef() != null && getR_PnRef().length() > 0) {
      if (!getR_PnRef().equals(documentNo)) setDocumentNo(getR_PnRef());
      return;
    }
  ...
  public void setAmount(int C_Currency_ID, BigDecimal payAmt) {
    if (C_Currency_ID == 0)
      C_Currency_ID = org.compiere.accounting.MClient.get(getCtx()).getC_Currency_ID();
  ...
  public void setBP_BankAccount(MBPBankAccount ba) {
    ...
    //	CC
    if (ba.getCreditCardType() != null) setCreditCardType(ba.getCreditCardType());
    if (ba.getCreditCardNumber() != null) setCreditCardNumber(ba.getCreditCardNumber());
    if (ba.getCreditCardExpMM() != 0) setCreditCardExpMM(ba.getCreditCardExpMM());
    if (ba.getCreditCardExpYY() != 0) setCreditCardExpYY(ba.getCreditCardExpYY());
    if (ba.getCreditCardVV() != null) setCreditCardVV(ba.getCreditCardVV());
    //	Bank
    if (ba.getAccountNo() != null) setAccountNo(ba.getAccountNo());
    if (ba.getRoutingNo() != null) setRoutingNo(ba.getRoutingNo());
    if (ba.getIBAN() != null) setIBAN(ba.getIBAN());
    if (ba.getSwiftCode() != null) setSwiftCode(ba.getSwiftCode());
  } //	setBP_BankAccount
  ...
  public boolean saveToBP_BankAccount(MBPBankAccount ba) {
    ...
    //	Bank
    if (getAccountNo() != null) ba.setAccountNo(getAccountNo());
    if (getRoutingNo() != null) ba.setRoutingNo(getRoutingNo());
    if (getIBAN() != null) ba.setIBAN(getIBAN());
    //	Trx
    ba.setR_AvsAddr(getR_AvsAddr());
    ba.setR_AvsZip(getR_AvsZip());
    //
    boolean ok = ba.save(null);
  ...
  public void setC_DocType_ID(boolean isReceipt) {
    ...
    try {
      ...
      rs = pstmt.executeQuery();
      if (rs.next()) setC_DocType_ID(rs.getInt(1));
  ...
  private boolean verifyDocType(MPaymentAllocate[] pAllocs) {
    if (getC_DocType_ID() == 0) return false;
    //
    ...
    if (documentSO != null && documentSO.booleanValue() != paymentSO.booleanValue()) return false;
    //	OK
    return true;
  } //	verifyDocType
  ...
  private boolean verifyPaymentAllocateVsHeader(MPaymentAllocate[] pAllocs) {
    if (pAllocs.length > 0) {
      if (getC_Charge_ID() > 0 || getC_Invoice_ID() > 0 || getC_Order_ID() > 0) return false;
    }
    return true;
  }
  ...
  public String prepareIt() {
    ...
    if (!MPaySelectionCheck.deleteGeneratedDraft(getCtx(), getC_Payment_ID(), null)) {
    ...
    if (isOnline() && !isApproved()) {
      if (getR_Result() != null) m_processMsg = "@OnlinePaymentFailed@";
  ...
  public CompleteActionResult completeIt() {
    ...
    if (isCashbookTrx()) {
      ...
      MCash cash =
          MCash.get(getCtx(),  getOrgId(), getDateAcct(), getC_Currency_ID(), null);
  ...
  private MPayment createCounterDoc() {
    ...
    //	Deep Copy
    MPayment counter = new MPayment(getCtx(), 0, null);
  ...
  public boolean allocateIt() {
    ...
    if (getC_Order_ID() != 0) return false;
    ...
    for (int i = 0; i < pAllocs.length; i++) {
      ...
      if (!aLine.save(null)) log.warning("P.Allocations - line not saved");
      else {
  ...
  private boolean allocateInvoice() {
    ...
    if (C_Project_ID > 0 && getC_Project_ID() == 0) setC_Project_ID(C_Project_ID);
      ...
      log.warning(
          "Invoice C_Project_ID=" + C_Project_ID + " <> Payment C_Project_ID=" + getC_Project_ID());
    return true;
  } //	allocateInvoice
  ...
  private boolean allocatePaySelection() {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        //
        if (alloc.getId() == 0 && !alloc.save(null)) {
        ...
        aLine.setC_Payment_ID(getC_Payment_ID());
        if (!aLine.save(null)) log.log(Level.SEVERE, "Could not create Allocation Line");
      }
    } catch (Exception e) {
    ...
    if (alloc.getId() == 0) {
      ...
      if (alloc.processIt(DocAction.Companion.getACTION_Complete())) ok = alloc.save(null);
      else
        throw new AdempiereException("Failed when processing document - " + alloc.getProcessMsg());
      // end added by zuhri
      m_processMsg = "@C_AllocationHdr_ID@: " + alloc.getDocumentNo();
  ...
  private void deAllocate(boolean accrual) {
    ...
    MAllocationHdr[] allocations =
        MAllocationHdr.getOfPayment(getCtx(), getC_Payment_ID(), null);
    if (log.isLoggable(Level.FINE)) log.fine("#" + allocations.length);
    for (int i = 0; i < allocations.length; i++) {
      allocations[i].set_TrxName(null);
  ...
  public boolean voidIt() {
    ...
    if (getC_BankStatementLine_ID() > 0) return reverseCorrectIt();
  ...
  private StringBuilder reverse(boolean accrual) {
    ...
    //	Create Reversal
    MPayment reversal = new MPayment(getCtx(), 0, null);
    ...
    aLine.setPaymentInfo(getC_Payment_ID(), 0);
    if (!aLine.save(null)) log.warning("Automatic allocation - line not saved");
    //	Reversal Allocation
    ...
    if (!aLine.save(null)) log.warning("Automatic allocation - reversal line not saved");
  ...
  public String getSummary() {
    ...
    sb.append(": ")
        .append(Msg.translate(getCtx(), "PayAmt"))
        .append("=")
        .append(getPayAmt())
        .append(",")
        .append(Msg.translate(getCtx(), "WriteOffAmt"))
        .append("=")
        .append(getWriteOffAmt());
    //	 - Description

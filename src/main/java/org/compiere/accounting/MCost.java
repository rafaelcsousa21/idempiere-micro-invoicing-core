...
public class MCost extends X_M_Cost {
  ...
  protected static void create(MProduct product) {
    ...
    MAcctSchema[] mass =
        MAcctSchema.getClientAcctSchema(
            product.getCtx(), product.getADClientID(), product.get_TrxName());
    MOrg[] orgs = null;
        ...
        if (as.getAD_OrgOnly_ID() > 0
          ...
          for (MOrg o : orgs) {
            ...
            if (as.getAD_OrgOnly_ID() == o.getAD_Org_ID() || as.getAD_OrgOnly_ID() == 0) {
              createCostingRecord(
                  product, M_ASI_ID, as, o.getAD_Org_ID(), ce.getM_CostElement_ID());
            }
          }
        }
   ...
   */
  public static BigDecimal calculateAverageInv(
      MProduct product, int M_AttributeSetInstance_ID, MAcctSchema as, int AD_Org_ID) {
    StringBuilder sql =
    ...
    try {
      ...
      while (rs.next()) {
        ...
        newAverageAmt =
            newAmt.divide(newStockQty, as.getCostingPrecision(), BigDecimal.ROUND_HALF_UP);
        if (s_log.isLoggable(Level.FINER))
          s_log.finer(
              "Movement="
                  + movementQty
   ...
   * @return costs or null
   */
  public static BigDecimal calculateFiFo(
      MProduct product, int M_AttributeSetInstance_ID, MAcctSchema as, int AD_Org_ID) {
    StringBuilder sql =
        new StringBuilder("SELECT t.MovementQty, mi.Qty, il.QtyInvoiced, il.PriceActual,")
            .append(
    ...
    try {
      ...
      while (rs.next()) {
        BigDecimal movementQty = rs.getBigDecimal(1);
        int M_Transaction_ID = rs.getInt(10);
        if (M_Transaction_ID == oldTransaction_ID) continue; // 	assuming same price for receipt
        M_Transaction_ID = oldTransaction_ID;
        //
        BigDecimal matchQty = rs.getBigDecimal(2);
        if (matchQty == null) // 	out (negative)
        {
          if (fifo.size() > 0) {
            ...
            if (remainder.signum() == 0) fifo.remove(0);
            else {
        ...
        if (fifo.size() == 1) {
          ...
          if (pp.Qty.signum() < 0) {
            pp.Qty = pp.Qty.add(movementQty);
            if (pp.Qty.signum() == 0) fifo.remove(0);
            else pp.Cost = cost;
            used = true;
          }
        }
   ...
   * @return costs or null
   */
  public static BigDecimal calculateLiFo(
      MProduct product, int M_AttributeSetInstance_ID, MAcctSchema as, int AD_Org_ID) {
    StringBuilder sql =
        new StringBuilder("SELECT t.MovementQty, mi.Qty, il.QtyInvoiced, il.PriceActual,")
            .append(
      ...
      while (rs.next()) {
        ...
        if (matchQty == null) // 	out (negative)
        {
          if (lifo.size() > 0) {
            ...
            pp.Qty = pp.Qty.add(movementQty);
            BigDecimal remainder = pp.Qty;
            if (remainder.signum() == 0) lifo.remove(lifo.size() - 1);
            else {
              while (remainder.signum() != 0) {
                if (lifo.size() == 1) // 	Last
                {
  ...
  public String toString() {
    ...
    if (getAD_Org_ID() != 0) sb.append(",AD_Org_ID=").append(getAD_Org_ID());
  ...
  public MCostElement getCostElement() {
    ...
    if (M_CostElement_ID == 0) return null;

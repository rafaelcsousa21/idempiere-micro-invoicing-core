...
public class MOrder extends org.compiere.order.MOrder implements DocAction, IPODoc {
  ...
  public MOrder(MProject project, boolean IsSOTrx, String DocSubTypeSO) {
    ...
    if (IsSOTrx) {
      ...
      else setC_DocTypeTarget_ID(DocSubTypeSO);
    } else setC_DocTypeTarget_ID();
  } //	MOrder
  ...
  protected boolean beforeSave(boolean newRecord) {
    ...
    if (newRecord || is_ValueChanged("AD_Org_ID") || is_ValueChanged("M_Warehouse_ID")) {
      if (wh.getAD_Org_ID() != getAD_Org_ID()) log.saveWarning("WarehouseOrgConflict", "");
    }
    ...
    if (getC_DocTypeTarget_ID() == 0) setC_DocTypeTarget_ID(DocSubTypeSO_Standard);
  ...
  public String prepareIt() {
    ...
    if (getC_DocType_ID() != getC_DocTypeTarget_ID()) {
      ...
      {
        if (dt.isOffer()) setC_DocType_ID(getC_DocTypeTarget_ID());
        else {
          m_processMsg = "@CannotChangeDocType@";
          return DocAction.Companion.getSTATUS_Invalid();
        }
    ...
    if (getGrandTotal().signum() != 0
      ...
      if (MOrderPaySchedule.getOrderPaySchedule(getCtx(), getC_Order_ID(), 0, get_TrxName()).length
          > 0) {
        m_processMsg = "@ErrorPaymentSchedule@";
        return DocAction.Companion.getSTATUS_Invalid();
      }
    ...
    if (isSOTrx()) {
      if (MDocType.DOCSUBTYPESO_POSOrder.equals(dt.getDocSubTypeSO())
          ...
          && !MSysConfig.getBooleanValue(
              MSysConfig.CHECK_CREDIT_ON_CASH_POS_ORDER, true, getADClientID(), getAD_Org_ID())) {
        ...
        MBPartner bp =
            new MBPartner(
                getCtx(),
                getBill_BPartner_ID(),
                get_TrxName()); // bill bp is guaranteed on beforeSave
  ...
  protected boolean reserveStock(MDocType dt, MOrderLine[] lines) {
    if (dt == null) dt = MDocType.get(getCtx(), getC_DocType_ID());
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (header_M_Warehouse_ID != 0) // 	enforce WH
      {
        ...
        if (getAD_Org_ID() != line.getAD_Org_ID()) line.setAD_Org_ID(getAD_Org_ID());
      }
      //	Binding
      ...
      if (product != null) {
        if (product.isStocked()) {
          ...
          if (!MStorageReservation.add(
              ...
              difference,
              isSOTrx,
              get_TrxName())) return false;
        } //	stocked
        //	update line
        line.setQtyReserved(line.getQtyReserved().add(difference));
        if (!line.save(get_TrxName())) return false;
        //
        Volume = Volume.add(product.getVolume().multiply(line.getQtyOrdered()));
        Weight = Weight.add(product.getWeight().multiply(line.getQtyOrdered()));
      } //	product
  ...
  public CompleteActionResult completeIt() {
    ...
    boolean realTimePOS =
        MSysConfig.getBooleanValue(MSysConfig.REAL_TIME_POS, false, getADClientID());
  ...
  protected String createPOSPayments() {
    ...
    List<X_C_POSPayment> pps =
        new Query(this.getCtx(), X_C_POSPayment.Table_Name, "C_Order_ID=?", this.get_TrxName())
    ...
    MBankAccount ba =
        new Query(this.getCtx(), MBankAccount.Table_Name, whereClause, this.get_TrxName())
    ...
    if (doctypes == null || doctypes.length == 0) return "No document type for AR Receipt";
    ...
    for (X_C_POSPayment pp : pps) {
      X_C_POSTenderType tt =
          new X_C_POSTenderType(getCtx(), pp.getC_POSTenderType_ID(), get_TrxName());
  ...
  public static MOrder copyFrom(
    ...
    return (MOrder)
        doCopyFrom(from, dateDoc, C_DocTypeTarget_ID, isSOTrx, counter, copyASI, trxName, to);
  }
  ...
  protected org.compiere.order.MOrder createCounterDoc() {
    ...
    if (getRef_Order_ID() != 0) return null;
    ...
    MOrder counter =
        copyFrom(
            this, getDateOrdered(), C_DocTypeTarget_ID, !isSOTrx(), true, false, get_TrxName());
    //
  ...
  public boolean voidIt() {
    ...
    if (getLink_Order_ID() > 0) {
      org.compiere.order.MOrder so =
          new org.compiere.order.MOrder(getCtx(), getLink_Order_ID(), get_TrxName());
  ...
  public String reopenIt() {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (Env.ZERO.compareTo(line.getQtyLostSales()) != 0) {
        ...
        if (!line.save(get_TrxName())) return "Couldn't save orderline";
      }
    }
    //	Clear Reservations
    if (!reserveStock(null, lines)) {
    ...
    setDocAction(DOCACTION_Close);
    if (!this.save(get_TrxName())) return "Couldn't save reopened order";
    else return "";
  } //	reopenIt
  /**

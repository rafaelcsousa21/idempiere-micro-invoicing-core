 ...
 * <p>BF [ 2789949 ] Multicurrency in matching posting
 */
public final class Fact implements IFact {
     ...
     */
    public FactLine createLine(
            DocLine docLine, MAccount accountDr, MAccount accountCr, int C_Currency_ID, BigDecimal Amt) {
        if (Amt.signum() < 0) return createLine(docLine, accountCr, C_Currency_ID, null, Amt.abs());
        else return createLine(docLine, accountDr, C_Currency_ID, Amt, null);
    } //  createLine
     ...
     *
     * @param segmentType - see AcctSchemaElement.SEGMENT_* Implemented only for Org Other sensible
     *                    candidates are Project, User1/2
     * @return true if segments are balanced
     */
    ...
    private void balanceSegment(String elementType) {
        ...
        if (elementType.equals(MAcctSchemaElement.ELEMENTTYPE_Organization)) {
            ...
            for (int i = 0; i < m_lines.size(); i++) {
                ...
                } else oldBalance.add(line.getAmtSourceDr(), line.getAmtSourceCr());
    ...
    public I_Fact_Acct balanceAccounting() {
        ...
        if (log.isLoggable(Level.FINE))
            log.fine(
                    "Balance="
                            + diff
                            + ", CurrBal="
                            + m_acctSchema.isCurrencyBalancing()
                            + " - "
                            + toString());
        FactLine line = null;
        ...
        if (m_acctSchema.isCurrencyBalancing()) {
            ...
            if (isDR) drAmt = difference;
            else crAmt = difference;
            //	Switch sides
            boolean switchIt =
                    BSline != null
            ...
            if (switchIt) {
                drAmt = Env.ZERO;
                crAmt = Env.ZERO;
                if (isDR) crAmt = difference.negate();
                else drAmt = difference.negate();
            }
        ...
        {
            if (BSline != null) line = BSline;
            else line = PLline;
            if (line == null) log.severe("No Line found");
            else {
    ...
    public boolean distribute() {
        ...
        for (int i = 0; i < m_lines.size(); i++) {
            ...
            for (I_GL_DistributionLine dl : lines) {
                ...
                if (dl.getDescription() != null) description += " - " + dl.getDescription();
    ...
    public static class Balance {
        ...
        public BigDecimal getPostBalance() {
            ...
            if (isReversal()) return bd.negate();
            return bd;
        } //	getPostBalance

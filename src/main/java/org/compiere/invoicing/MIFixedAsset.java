...
public class MIFixedAsset extends X_I_FixedAsset {
  ...
  private void fixKeyValue(int idx) {
    ...
    if (log.isLoggable(Level.FINE))
      log.fine(
          getInventoryNo() + ": " + get_ColumnName(idx) + "=[" + name + "]->[" + newName + "]");
  ...
  public void process() {
    ...
    try {
      if (getUseLifeMonths() <= 0) {
        throw new FillMandatoryException(I_I_FixedAsset.COLUMNNAME_UseLifeMonths);
      }
      /*//comment by @win
      ...
      */
      ...
      for (int idx = 0; idx < col_count; idx++) {
        ...
        if (DisplayType.Amount == dt) fixAmount(idx);
        else if (DisplayType.isText(dt)) fixKeyValue(idx);
      }
      ...
      if (getM_Product_ID() <= 0) {
        throw new FillMandatoryException(I_I_FixedAsset.COLUMNNAME_M_Product_ID);
      }
      ...
      if (A_Asset_Group_ID > 0) {
        ...
        throw new FillMandatoryException(I_I_FixedAsset.COLUMNNAME_A_Asset_Group_ID);
      }
      ...
      if (getDateAcct() == null) {
        throw new FillMandatoryException(I_I_FixedAsset.COLUMNNAME_DateAcct);
      }

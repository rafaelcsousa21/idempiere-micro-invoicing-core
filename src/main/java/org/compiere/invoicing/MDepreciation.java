...
public class MDepreciation extends X_A_Depreciation {
            ...
            if (log.isLoggable(Level.FINE))
                log.fine(
                        "Entering: DepreciationType="
                                ...
                                + assetAcct
                                + ", A_Current_Period="
                                + A_Current_Period // + " (offset=" + offset + ")"
                                + ", Accum_Dep="
                                + Accum_Dep);
        }
    ...
    public boolean canInvoke(
        ...
        int offset =
                0; // getFixMonthOffset(); this field does not exist in AD nor DB but exists in X_. - red1
     ...
     * @return Env.ZERO
     */
    private BigDecimal apply_ARH_ZERO(
     ...
     * @return depreciation for the current month
     */
    private BigDecimal apply_ARH_VAR(
            MDepreciationWorkfile wk, MAssetAcct acct, int A_Current_Period, BigDecimal Accum_Dep) {
        BigDecimal amt = wk.getActualCost();
        BigDecimal varPercent =
                acct.getA_Depreciation_Variable_Perc(wk.isFiscal())
    ...
    */
        ...
        if (log.isLoggable(Level.FINE)) {
            ...
            // log.fine("coef_K=" + coef_K + ", coef_sl=" + coef_sl + ", coef_ad1=" + coef_ad1);
        ...
        for (int curr_year = 0; curr_year <= A_Current_Year; curr_year++) {
            if (!is_SL) {
                ...
                amt_sl =
                        amt_r.divide(
                                new BigDecimal(A_RemainingLife_Year), getPrecision(), RoundingMode.HALF_UP);
                // logging
                if (CLogMgt.isLevelFinest()) {
                    if (s_log.isLoggable(Level.FINE))
                        s_log.fine(
                                "amt_r="
                                        + amt_r
                                        + ", amt_ad1=amt_r*coef_ad1="
                                        + amt_ad1
                                        + ", amt_sl=amt_r/A_RemainingLife_Year="
                                        + amt_sl
                                        + ", A_Current_Year="
                                        + A_Current_Year
            ...
            if (s_log.isLoggable(Level.FINE))
                s_log.fine(
                        "year=" + curr_year + ", amtPerYear=" + amtPerYear + ", amt_r=" + amt_r); // logging
        }
        if (s_log.isLoggable(Level.FINE))
     ...
     *
     */
    private BigDecimal apply_ARH_AD2(
            MDepreciationWorkfile wk, MAssetAcct assetAcct, int A_Current_Period, BigDecimal Accum_Dep) {
  ...
  /*private static BigDecimal get_AD_K(int A_Life_Year)
  {
     ...
     * errors from the adjustment calculation
     *

...
public class MInventory extends X_M_Inventory implements DocAction, IPODoc {
  ...
  public MInventoryLine[] getLines(boolean requery) {
    ...
    List<MInventoryLine> list =
        new Query(getCtx(), I_M_InventoryLine.Table_Name, "M_Inventory_ID=?", null)
            .setParameters(getId())
        ...
        if (product != null && product.isASIMandatory(line.isSOTrx())) {
          if (!product
            ...
            MInventoryLineMA mas[] =
                MInventoryLineMA.get(getCtx(), line.getM_InventoryLine_ID(), null);
      ...
      try {
        ...
        if (!isReversal()) {
          BigDecimal qtyOnLineMA =
              MInventoryLineMA.getManualQty(line.getM_InventoryLine_ID(), null);
  ...
  private void checkMaterialPolicy(MInventoryLine line, BigDecimal qtyDiff) {
    ...
    int no = MInventoryLineMA.deleteInventoryLineMA(line.getM_InventoryLine_ID(), null);
  ...
  private MInventory reverse(boolean accrual) {
    ...
    //	Deep Copy
    MInventory reversal = new MInventory(getCtx(), 0, null);
    ...
    for (int i = 0; i < oLines.length; i++) {
      ...
      MInventoryLine rLine = new MInventoryLine(getCtx(), 0, null);
      ...
      if (rLine.getMAttributeSetInstance_ID() == 0) {
        MInventoryLineMA mas[] =
            MInventoryLineMA.get(getCtx(), oLines[i].getM_InventoryLine_ID(), null);
        for (int j = 0; j < mas.length; j++) {
          MInventoryLineMA ma =
              new MInventoryLineMA(
                  rLine,

...
public abstract class AdempiereServer implements Runnable {
  ...
  protected AdempiereServer(AdempiereProcessor model, int initialNap) {
    ...
    p_client = MClient.get(model.getCtx(), model.getClientId());
    m_initialNap = initialNap;

    Timestamp dateNextRun = getDateNextRun(true);
    if (dateNextRun != null) m_nextWork = dateNextRun.getTime();
  ...
  public void runNow() {
    ...
    MClient schedclient = MClient.get(getCtx(), p_model.getClientId());
    Env.setContext(context, "#AD_Client_ID", schedclient.getClientId());
  ...
  private int getAD_User_ID(PO po) {
    int AD_User_ID = -1;
    if (po.get_ValueAsInt("Supervisor_ID") > 0) AD_User_ID = po.get_ValueAsInt("Supervisor_ID");
    else if (po.get_ValueAsInt("CreatedBy") > 0) AD_User_ID = po.get_ValueAsInt("CreatedBy");
    else if (po.get_ValueAsInt("UpdatedBy") > 0) AD_User_ID = po.get_ValueAsInt("UpdatedBy");
  ...
  public void doRunNow() {
    ...
    p_model.saveEx();
    //
    if (log.isLoggable(Level.FINE)) log.fine(getStatistics());
  } //	runNow
  ...
  public void run() {
    ...
    Properties context = new Properties();
    Env.setContext(context, "#AD_Client_ID", p_model.getClientId());
    if (p_model instanceof PO) {
      PO po = (PO) p_model;
      if (po.get_ColumnIndex("AD_Org_ID") >= 0)
        Env.setContext(context, "#AD_Org_ID", po.get_ValueAsInt("AD_Org_ID"));

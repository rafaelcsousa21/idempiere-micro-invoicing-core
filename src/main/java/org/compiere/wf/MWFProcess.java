...
public class MWFProcess extends X_AD_WF_Process {
  ...
  */
    ...
    /**
     * ************************************************************************ Check Status of
     * Activities. - update Process if required - start new activity
     *
     * @param trxName transaction
     */
    public void checkActivities(String trxName, PO lastPO) {
        ...
        for (int i = 0; i < activities.length; i++) {
            ...
            if (activityState.isClosed()) {
                ...
                activity.saveEx();
                //
                if (closedState == null) closedState = activityWFState;
                else if (!closedState.equals(activityState.getState())) {
                    //	Overwrite if terminated
                    if (activityState.isTerminated()) closedState = activityWFState;
                        //	Overwrite if activity aborted and no other terminated
                    else if (activityState.isAborted()
                            && !X_AD_WF_Process.WFSTATE_Terminated.equals(closedState))
                        closedState = activityWFState;
                }
            } else //	not closed
            {
                closedState = null; // 	all need to be closed
                if (activityState.isSuspended()) suspended = true;
                if (activityState.isRunning()) running = true;
            }
        } //	for all activities
    ...
    private boolean startNext(MWFActivity last, MWFActivity[] activities, PO lastPO) {
        ...
        for (int i = 0; i < transitions.length; i++) {
            ...
            if (MWFNode.SPLITELEMENT_XOR.equals(split)) return true;
        } //	for all transitions
        return true;
    } //	startNext
    ...
    /**
     * Set User from - (1) Responsible - (2) Document Sales Rep - (3) Document UpdatedBy - (4) Process
     * invoker
     *
    ...
    private void setUser_ID(Integer User_ID) {
        ...
        if (AD_User_ID == 0 && resp.isInvoker()) {
            ...
            //	(3) UpdatedBy
            if (AD_User_ID == 0 && m_po != null) AD_User_ID = m_po.getUpdatedBy();
        }
    ...
    public void setTextMsg(PO po) {
        if (po != null && po instanceof DocAction) setTextMsg(((DocAction) po).getSummary());
    } //	setTextMsg
    ...
    public void setTextMsg(String TextMsg) {
        ...
        if (oldText == null || oldText.length() == 0) super.setTextMsg(TextMsg);
        else if (TextMsg != null && TextMsg.length() > 0) super.setTextMsg(oldText + "\n - " + TextMsg);
    } //	setTextMsg
    ...
    public void setProcessMsg(String msg) {
        m_processMsg = msg;
        if (msg != null && msg.length() > 0) setTextMsg(msg);
    } //	setProcessMsg
} //	MWFProcess
